...
const schema = z.object({
  ...
  patternOffsetX: z.number().optional(),
  patternOffsetY: z.number().optional(),
  patternRotation: z.number().optional(),
  repeatMode: z.enum(["REPEAT", "COVER", "CONTAIN"]).optional(),
  usableAsTexture: z.boolean().optional(),
...
export async function POST(req: Request) {
  ...
  const fabric = await prisma.fabric.create({
    data: {
      ...
      textureImage: parsed.data.textureImage || null,
      patternScale: parsed.data.patternScale ?? 1,
      patternOffsetX: parsed.data.patternOffsetX ?? 0,
      patternOffsetY: parsed.data.patternOffsetY ?? 0,
      patternRotation: parsed.data.patternRotation ?? 0,
      repeatMode: parsed.data.repeatMode ?? "REPEAT",
      usableAsTexture: parsed.data.usableAsTexture ?? false,
